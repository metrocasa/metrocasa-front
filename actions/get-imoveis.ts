...
const populate =
  'populate[planta_comp][populate][fields][0]=url&&populate[evolucao_obras][populate][gallery][fields][0]=url&populate[main_gallery][populate]=*&populate[fachada][populate][fields][0]=url&populate[logo][fields][0]=url&populate[materiais][populate][a3][fields][0]=*&populate[materiais][populate][fase_1][fields][0]=*&populate[materiais][populate][fase_2][fields][0]=*&populate[materiais][populate][fase_3][fields][0]=*&populate[materiais][populate][books][fields][0]=*&populate[materiais][populate][ri][fields][0]=*';
...
export const searchImoveis = async (search: string) => {
  try {
    const res = await axios.get(
      `${BASE_URL}/api/imoveis?filters[title][$contains]=${search}&${populate}`,
      {
        headers: {
          Authorization: `Bearer ${TOKEN}`,
        },
